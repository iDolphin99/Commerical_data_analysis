...
def preprocessing_for_ensemble(data) :
    ...
    # Binary Encoding 
    be = pickle.load(open("./model/binaryEncoder.pkl", "rb"))
...
def prediction(data_ml, data_dl) : 
    ...
    # Load model 
    model_ml = pickle.load(open('./model/model_ml.h5', 'rb'))
    model_dl = tf.keras.models.load_model('./model/model_dl.h5')
    ...
    pred_ensemble_prob = pred_ml_prob * 0.8 + pred_dl_prob * 0.2
...
def get_range_from_label(pred) : 
    #currentPath = os.getcwd()
    label_df = pd.read_csv("./model/df_label.csv" ,index_col = 0)
...
if __name__ == '__main__' :
    ...
    #print(pred_low)
    print(pred[0], pred_low[0], pred_up[0], end='')
    #print(pred_up)
